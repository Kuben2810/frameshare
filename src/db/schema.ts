...
export const workspaces = pgTable("workspaces", {
  ...
  plan:                 text("plan", { enum: ["free", "solo", "pro", "studio"] }).notNull().default("free"),
  planStatus:           text("plan_status", { enum: ["active", "past_due", "cancelled"] }).notNull().default("active"),
  planExpiresAt:        timestamp("plan_expires_at", { mode: "date" }),
  payfastToken:         text("payfast_token"),
  storageQuotaBytes:    bigint("storage_quota_bytes", { mode: "number" }).notNull().default(2_147_483_648),
  storageUsedBytes:     bigint("storage_used_bytes", { mode: "number" }).notNull().default(0),
